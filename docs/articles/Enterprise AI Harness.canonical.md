...
Покажу на конкретном примере, как четыре слоя работают в одном запросе. В качестве сценария возьмём workflow разработки с AI-агентами: задача приходит в систему как task, затем распределяется между агентами, проходит через последовательный flow и завершается review результата, как это обычно описывают в публичных agentic workflow-подходах, например в этой статье 

Возьмём паттерн из статьи ([AI Agents & Agentic Workflows](https://medium.com/towards-applied-generative-ai/ai-agents-agentic-workflows-f558674ee18b)). Задача приходит как task, затем распределяется между агентами, проходит через workflow и завершается review результата. В качестве конкретного примера рассмотрим инженерную задачу. "добавить новую валидацию в API-метод, обновить тесты и подготовить итоговый diff для review".

![Enterprise AI Harness Architecture](../../diagrams/enterprise-ai-harness-workflow.png)
